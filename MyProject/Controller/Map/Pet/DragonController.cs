...
public class DragonController : MonoBehaviour
{
    ...
    public float maxSpeed = 1f; //0 Walking, 1 Running
...
//        Stun = Input.GetMouseButton(2);
    ...
    bool isJumping(float normalizedtime, bool half)
    {
        if (half)  //if is jumping the first half
        {
            ...
            {
                if (anim.GetCurrentAnimatorStateInfo(0).normalizedTime < normalizedtime) return true;
            }
            if (anim.GetNextAnimatorStateInfo(0).IsTag("Jump"))
            {
                if (anim.GetNextAnimatorStateInfo(0).normalizedTime < normalizedtime) return true;
            }
        }
    ...
    bool isJumping()
    {
        if (anim.GetCurrentAnimatorStateInfo(0).IsTag("Jump")) { return true; }
        if (anim.GetNextAnimatorStateInfo(0).IsTag("Jump")) { return true; }
        return false;
    }
