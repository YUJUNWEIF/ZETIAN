...
public class FingerGestures : MonoBehaviour
{
    ...
    public class FingerList : IFingerList
    {
        ...
        public bool MovingInSameDirection( float tolerance )
        {
			...
			float refDistance = refDir.magnitude;
			if(refDistance < 1f) { return false; }
			refDir /= refDistance;

			refDistance = FingerGestures.Convert(refDistance, DistanceUnit.Pixels, DistanceUnit.Centimeters);
            ...
            for( int i = 1; i < Count; ++i )
            {
				...
				distance = FingerGestures.Convert(distance, DistanceUnit.Pixels, DistanceUnit.Centimeters);
//				if (Mathf.Abs(refDistance - distance) > 0.25f)
//				{
//					return false;
//				}
