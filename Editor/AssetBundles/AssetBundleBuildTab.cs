...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private void ExecuteBuild()
        {
            ...
            After(m_UserData.m_OutputPath);
        }
        static void After(string outputPath)
        {
            //using (WWW www = new WWW("file:///" + Application.streamingAssetsPath +"/" + outputPath))
            //{
            //    var manifest = www.assetBundle.LoadAsset<AssetBundleManifest>("AssetBundleManifest");
            //    var pp = Convert(manifest);
            //    var files = Directory.GetFiles(outputPath, "*.unity3d", SearchOption.AllDirectories);
            //    for (int index = 0; index < files.Length; ++index)
            //    {
            //        var fs = File.OpenRead(files[index]);


            //        var filePath = files[index].Replace('\\', '/');

            //        var fileName = filePath.Substring(outputPath.Length + 1);
            //        var hash = pp.hashs.Find(it => it.name == fileName);
            //        if (hash != null)
            //        {
            //            var file = new FileInfo(filePath);
            //            hash.size = (int)file.Length;
            //        }
            //    }
            //    var buffer = new byte[4 * 1024 * 1024];
            //    var pos = pp.Marsh(buffer, 0);

            //    var p1 = outputPath.LastIndexOf('\\');
            //    var p2 = outputPath.LastIndexOf('/');
            //    var p = (p1 > p2) ? p1 : p2;
            //    var name = p >= 0 ? outputPath.Substring(p) : outputPath;
            //    var of = Path.Combine(outputPath, name + ".assetInfo");
            //    if (File.Exists(of)) { File.Delete(of); }
            //    var fp = new FileStream(of, FileMode.CreateNew);
            //    fp.Write(buffer, 0, pos);
            //    fp.Close();
            //}
            stop = false;
            //ThreadPool.QueueUserWorkItem(o =>
            {
                var metaInfos = new List<AssetMetaInfo>();
                var files = Directory.GetFiles(outputPath, "*.unity3d", SearchOption.AllDirectories);
                for (int index = 0; index < files.Length && !stop; ++index)
                {
                    var fs = File.OpenRead(files[index]);
                    var md5 = Util.Crypt.GetMD5_32(fs);
                    metaInfos.Add(new AssetMetaInfo() { name = fs.Name, md5 = md5, size = (int)fs.Length });                    
                }

                var pp = new AssetMetaManifest(outputPath, metaInfos);
                var buffer = new byte[4 * 1024 * 1024];
                var pos = pp.Marsh(buffer, 0);
                var p1 = outputPath.LastIndexOf('\\');
                var p2 = outputPath.LastIndexOf('/');
        ...
        static AssetMetaManifest Convert(AssetBundleManifest manifest)
        {
            List<AssetMetaInfo> hashs = new List<AssetMetaInfo>();
            var bundles = manifest.GetAllAssetBundles();
            for (int index = 0; index < bundles.Length; ++index)
            {
                var hash = manifest.GetAssetBundleHash(bundles[index]);
                hashs.Add(new AssetMetaInfo() { name = bundles[index], md5 = hash.ToString(), size = 0 });
            }
            return new AssetMetaManifest(manifest.name, hashs);
        }
