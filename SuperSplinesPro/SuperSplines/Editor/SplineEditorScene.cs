...
public partial class SplineEditor : InstantInspector
{
	...
	private void HandleMouseInput( Spline spline )
	{
		...
		FESplineNode selectedNode = null;
		
		foreach( var node in spline.SplineNodes )
		{
			float newDistance = Vector3.Distance( node.position.V3(), position );
			
			if( newDistance < currentDistance || newDistance < 0.2f * HandleUtility.GetHandleSize( node.position.V3() ) )
			{
		...
		{
			position = selectedNode.position.V3();
			...
			Handles.Label( LabelPosition2D( position, 0.3f ), "Delete Node (" + (selectedNode.unityUse as SplineNode).gameObject.name + ")", sceneGUIStyleToolLabel ); 
		...
		{
			Undo.RegisterSceneUndo( ( selectedNode != null ) ? "Delete Spline Node (" + (selectedNode.unityUse as SplineNode).name +  ")" : "Insert Spline Node" );
			
			if( selectedNode != null )
			{
				spline.RemoveSplineNode((selectedNode.unityUse as SplineNode).gameObject );
				DestroyImmediate((selectedNode.unityUse as SplineNode).gameObject );
			}
			else
	...
	private void DrawHandles( Spline spline )
	{
		...
		foreach( var no in spline.SplineNodes )
		{
            var node = (no.unityUse as SplineNode);
	...
	{	
		if( Event.current.alt && !Event.current.shift )
		{
            foreach (var no in spline.SplineNodes)
            {
                var splineNode = no.unityUse as SplineNode;
                Handles.Label(LabelPosition2D(splineNode.Position, -0.2f), splineNode.name, sceneGUIStyle);
            }
			
			foreach( FESplineSegment segment in spline.SplineSegments )
			{
				for( int i = 0; i < 10; i++ )
				{
					float splineParam = (float)segment.ConvertSegmentToSplineParamter( i/10f );
		...
		{
			foreach( FESplineSegment item in spline.SplineSegments )
			{
				Vector3 positionOnSpline = spline.GetPositionOnSpline( (float) item.ConvertSegmentToSplineParamter( 0.5f ) );
	...
	private void InsertNode( Spline spline, float splineParam )
	{
		...
		FESplineSegment segment = spline.GetSplineSegment( splineParam );
		
		int startNodeIndex = spline.splineNodesArray.IndexOf( segment.StartNode.unityUse  as SplineNode);
