...
public class CarAnimator : MonoBehaviour 
{
	...
	void Update( ) 
	{
        ...
        if (!forward) clampedParam = 1 - clampedParam;
        ...
        transform.rotation = spline.cacheImpl.GetOrientationOnSpline(WrapValue(passedTime + rotationOffset, 0f, 1f, wrapMode)).QU();
        transform.position = spline.cacheImpl.GetPositionOnSpline(clampedParam).V3();// - transform.right * spline.GetCustomValueOnSpline(clampedParam) * .5f;
    }
